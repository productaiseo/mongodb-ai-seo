...
const VerifyEmail = () => {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/auth/resend-verification`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      alert(
        "Doğrulama e-postası başarıyla gönderildi! Lütfen gelen kutunuzu kontrol edin."
      );
    } catch (err) {
      alert(
        err instanceof Error
      ...
      <div className="w-full max-w-md mx-auto px-4">
        {status === "loading" && (
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-white">Lütfen bekleyiniz...</p>
          </div>
        )}
